...
1. **Establish the target.** Read the authoring standard. When the request is sparse or names a familiar graphic category without enough implementation detail, read [references/briefing.md](references/briefing.md), select exactly one matching card from [references/archetypes/archetypes.md](references/archetypes/archetypes.md), and compile a semantic Composer Build Brief before discovering live schemas or fonts. Record safe assumptions and continue; ask only when a missing decision materially changes composition boundaries, required controls, runtime behavior, or requested branding. Then run `inspect`, confirm `activeComposition.stack`, read every existing target that may change, and reconcile inspect-dependent context such as placement and coexistence into the brief. Capture an untouched baseline only when the existing appearance must be preserved or compared, a supplied reference needs before/after evidence, or a rendering defect is being diagnosed. When an inspected target is structurally empty and no comparison is needed, skip capture and record `baseline: { "status": "baseline-not-applicable", "reason": "empty-target" }`.
2. **Plan the composition.** Use the build brief to decide the tile, group, and sub-composition boundaries; public controls; dynamic-value requirements; layer order; and element lifecycles before mutating. For reference motion, read [references/video-reference.md](references/video-reference.md). When the request matches a reusable effect, select the relevant card from [references/techniques/techniques.md](references/techniques/techniques.md). Route Table, AISVG, widget-template, text/font, and script work to their matching references instead of improvising their contracts. Archetype cards define what makes the requested graphic complete; technique cards define reusable construction or motion decisions; the live Composer schema remains authoritative for what can actually be authored.
...
| [references/briefing.md](references/briefing.md) | Compile a sparse request into the semantic Composer Build Brief used to plan construction and acceptance. |
| [references/archetypes/archetypes.md](references/archetypes/archetypes.md) | Select exactly one graphic archetype card for a sparse or category-level request. |
