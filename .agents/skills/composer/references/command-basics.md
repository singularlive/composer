...
| Command | Purpose |
| --- | --- |
| `doctor [--capture] [--connection <name>]` | Diagnose package/protocol versions, selected installation scope and path, duplicate project/global copies, bundled core and required Playwright readiness, and optionally system Chrome and the paired server version. It does not acquire a work lease. |
| `pair --connection <name> [--server <url>] --code <code>` | Preflight the isolated profile, claim a one-time pairing code, atomically store a 30-day scene/user JWT authorization, and acknowledge that exact authorization in Composer. An invalid, expired, or already-used code fails immediately with `PAIRING_CODE_INVALID`; do not retry it or switch to `pair-intent`, and request a fresh visible code. Success requires both `paired: true` and `acknowledged: true`. A failed receipt keeps `paired: true` but returns `acknowledgement.status: "failed"` with only `version-mismatch`, `authorization-rejected`, `timeout`, `editor-unavailable`, or `acknowledgement-rejected`; raw socket errors and credentials are omitted. The server defaults to `https://beta.singular.live/`; output also reports the sanitized `credentialStorage` category. |
...
| `wait-ready [--timeout <milliseconds>]` | Wait until authorization, editor connection, editor command registration, and the work lease are all ready. Returns immediately when ready. An absent editor fails after a two-second cross-process probe grace; a connected editor that is still initializing or lacks a work lease may use the default 30-second overall timeout. The range is 1–120000 ms. It does not mutate, reload, navigate, or renew the lease. |
...
After `start-work`, run `wait-ready` before the first editor command. A successful result has `status: "ready"`, `authorization: "active"`, `editor: "connected"`, `commands: "ready"`, and `workLease: "active"`. An absent editor exits quickly with `COMPOSER_EDITOR_DISCONNECTED` and `status: "editor-disconnected"`; release the lease and ask the user to reopen the paired composition. A connected but initializing editor or missing lease may reach the overall timeout and return `COMPOSER_NOT_READY` with `status: "timeout"`. Do not retry either result with a longer timeout, substitute repeated `inspect` calls, reload automatically, or tell the user to open or foreground the Composer AI panel.
