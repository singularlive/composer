...
| `check-connection [--timeout <milliseconds>]` | Prove that saved authorization is active, the editor is connected, and its command handler is ready without acquiring or requiring a work lease. Success returns `status: "connected"` and reports `workLease` separately. A relay-confirmed stale browser panel fails immediately with `EDITOR_RELOAD_REQUIRED`; reload the Composer AI panel and retry because pairing persists. It has the same 1–120000 ms bounded timeout and sanitized diagnostics as `wait-ready`. |
