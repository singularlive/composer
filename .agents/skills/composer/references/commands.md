...
Pipe that JSON to `scripts/singularTokenScriptCli.js` through stdin with `--handoff-file -`; use a path only for an intentionally managed short-lived handoff. The handoff suggests the active composition as the script target and carries the scene/account-scoped authorization required by every dedicated script endpoint, so `get-script`, `put-script`, and `clear-script` do not require `--script-id` unless the caller intentionally overrides it. Explicit disconnection, `complete`, and credential expiry prevent later script access. Use `summary --full` with the same handoff for global, overlay, ambiguous, or out-of-scope discovery. Direct `--token` and `--host` operation is unsupported. See [composition-scripts.md](composition-scripts.md).
