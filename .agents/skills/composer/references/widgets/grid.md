...
The command verifies Grid identity and the current template, writes options before canonical `tableContent` (`JSON.stringify({content: items})`), and reconciles attempted writes from authoritative readback before compensating in reverse order after failure. Shrinking dimensions precede growing dimensions to avoid temporary allocation overflow. It never changes `composition`. Its response contains `grid`, `widgetSubComposition`, item count `rows`, verified `options`, and `tableContent`. As with Table, this is a CLI transaction across relay writes, not a single editor undo batch. The [Table recovery contract](table.md) applies: cancellation stops further commands; unavailable, conflicting or failed recovery returns `TABLE_UPDATE_RECOVERY_UNCERTAIN` and preserves potentially partial state. Follow "Mutation failure recovery" in [commands.md](../commands.md) rather than assuming isolation or replaying after cancellation.
