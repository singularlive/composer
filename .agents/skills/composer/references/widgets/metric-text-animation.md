...
3. Re-read the tile after changing the effect. Only then set its newly advertised parameters, such as `inMoveDistance`. Batch several already-discovered fields with `set-properties --file <manifest.json>`; do not batch an effect selection together with undiscovered dependent fields. Repeat discovery after conditional selections change. Hidden saved parameters may remain in `values`; their presence does not mean they are currently active.
