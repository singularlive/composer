...
Observed after a Table Control Node link/unlink: stored `tableContent` became an array. `update-table` preserves the inspected field shape: arrays receive row arrays, and strings receive `JSON.stringify({content: rows})`. Unsupported stored shapes fail before dispatch; the command does not normalize the editor model to a different type. Readback and recovery compare structured values by content, not array identity. Runtime `setPayload` acceptance remains distinct from the editor's type-preserving setter.
...
If recovery confirms restoration, the original failure is still reported. When authoritative readback confirms every affected field is unchanged, the failure explicitly says `readback confirms no change; no compensation needed`; a rejected write on array content no longer becomes uncertain solely because readback returns a new array instance. This describes observed state, not proof that no transient write ever occurred. Unavailable or conflicting readback, changed identity, or failed compensation stops recovery with `TABLE_UPDATE_RECOVERY_UNCERTAIN`; this code is shared by Table and Grid and does not imply no mutation occurred. Preserve the observed state and follow "Mutation failure recovery" in [commands.md](../commands.md) before any retry. After cancellation, report potentially unrestored state without reconnecting. Readback is not compare-and-set: late in-flight writes or changes between read and write remain possible, so do not claim transactional isolation or automatic recovery from every disconnection.
