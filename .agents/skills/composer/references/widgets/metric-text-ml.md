...
The `font` field is `metricfont`, containing a complete Font 2.0 `fontData` object and metrics (`mg`). Use `set-metric-font` for an unlinked field and `set-control-font` for its defining linked control; do not fabricate metrics, replace it with a family-name string, or use ordinary Text `set-font`. Choose the font before final alignment and spacing. Font size follows available widget height, the selected line count, line spacing, and font metrics. Resize the placement rather than inventing a font-size property. Preserve operator-entered casing with `transform: "none"` unless another transform is explicitly requested.
