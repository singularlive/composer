...
When the user explicitly requests Font 2 migration, inspect the complete legacy ticker tile and run `upgrade-metric-widgets --ids <tile-id>`. The command converts Simple Ticker to Metric Text Ticker while preserving tile identity, layout, animation, grouping, and compatible same-field links. Reinspect the result and verify continuous scrolling in the Player. Do not migrate solely because Metric Text Ticker is preferred for new work; incompatible links reject the batch, and `fontMapped: false` requires a deliberate Metric Font selection.
