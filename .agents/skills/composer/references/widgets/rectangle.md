...
Rectangle applies one bevel style and size to all four corners. It cannot natively make only the right corners rounded. For a square-left/rounded-right panel, use two same-color layers: a full rounded Rectangle for the complete panel, then a square Rectangle patch above its left edge. Size the patch to cover the left corner arcs without reaching the right radius, align both layers exactly on the outer bounds, and keep the patch directly above the rounded base but behind content. Prefer a solid fill. With gradients, two independent widgets restart their gradients and can expose a seam; use one Gradient or AI Graphics surface only when continuous gradient pixels are an explicit requirement and that primitive's tradeoffs are accepted.
