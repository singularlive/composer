...
10. Verify in the Singular Player. Ensure `playwright-core@1.63.0` resolves from the bundled verifier and Google Chrome is installed on the target machine, then pipe a fresh handoff to `scripts/verifyComposition.mjs --handoff-file -`. Add `--scenario-file <path>` for version-1 declarative Player actions, bounded waits, lifecycle/state/DOM assertions, and named capture checkpoints. The verifier uses the installed Chrome channel; copy it to `temp/` only when the required trigger or assertion cannot be expressed by the declarative contract. Do not use a Composer canvas screenshot as runtime proof.
