...
10. Verify in the Singular Player. Ensure `@playwright/cli` and its Chrome browser are installed, then pipe a fresh handoff to `scripts/verifyComposition.mjs --handoff-file -`. Add `--scenario-file <path>` for version-1 declarative Player actions, bounded waits, lifecycle/state/DOM assertions, and named capture checkpoints. The bundled verifier runs in place against `playwright-core`; copy it to `temp/` only when the required trigger or assertion cannot be expressed by the declarative contract. Do not use a Composer canvas screenshot as runtime proof.
