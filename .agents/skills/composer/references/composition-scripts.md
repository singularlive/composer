...
Successful `put-script` and `clear-script` operations publish one sanitized Composer AI activity entry classifying the result as created, updated, cleared, or unchanged. For bounded scripts the entry includes added and removed line counts; larger scripts fall back to the resulting line count. Structured audit metadata additionally records before/after line and byte counts, duration, write mode, outcome, and Control App reload-notification status. Neither channel includes script text, changed line contents, credentials, URLs, or a script-content fingerprint.

The helper rejects direct `--token` and `--host` operation, omits both handoff credentials from normal output, and redacts token-bearing URL segments from request errors. Every script request carries the handoff's Composer Agent version; a missing or mismatched version is rejected before work-lease renewal. Missing or invalid authorization returns 401. The helper preserves stable `COMPOSER_AGENT_VERSION_MISMATCH`, `SESSION_CANCELLED`, `SESSION_COMPLETED`, `OPERATION_CANCELLED`, and `WORK_NOT_STARTED` codes from JSON error responses without exposing response text. A revoked, completed, expired, scene-mismatched, or account-mismatched authorization cannot access scripts. `OPERATION_CANCELLED` blocks further work under the current lease without revoking the saved credential; stop the current task and discard its handoff instead of issuing another script request. An expired or never-started lease reports `WORK_NOT_STARTED`. Return to the paired phase, establish a fresh work lease and handoff, and do not blindly replay a write with an uncertain outcome.
...
10. When verification uses an existing managed Control App, update its composition from an authenticated app view opened through Composer before testing the new script. Do not look for or attempt this action in a shared access-token URL. Confirm the app moved to a newly generated composition extract; ordinary page, output, and Player iframe refreshes only reload the current extract.
11. Verify in the Singular Player. Ensure `playwright-core@1.63.0` resolves from the bundled verifier and Google Chrome is installed on the target machine, then pipe a fresh handoff to `scripts/verifyComposition.mjs --handoff-file - --out <task-dir>`. Add `--scenario-file <path>` for version-1 declarative Player actions, bounded waits, lifecycle/state/DOM assertions, and named capture checkpoints. The verifier uses the installed Chrome channel; use a separate task-temporary harness only when the required trigger or assertion cannot be expressed by the declarative contract. Do not use a Composer canvas screenshot as runtime proof.
12. `composer-agent capture --wait-mode timed --settle <seconds>` opens the persisted public Player route and can provide a passive visual capture of the script result without requiring the scripted output to become still. Choose the bounded settle time from the initialization or data contract. It cannot by itself prove an event-driven path that was never triggered.
...
For clocks, counters, scores, and other script-driven text in a newly authored composition, use the matching Metric Text family widget first. A legacy Text widget remains appropriate when extending a composition that already uses legacy Text. Resolve the widget once by its stable Composer name, call `widget.setPayload({ text: value })` only when the displayed string changes, and clear the scheduled update in `close()`. Use AI Graphics generated fields when changing labels are inseparable from one coherent programmable graphic; AI Graphics is not a general replacement for independently editable text widgets.
...
- Treat Composer AI script activity as an operational audit summary, not proof of script correctness. The Player remains authoritative for runtime behavior.
...
- Store working scripts, custom harnesses, sanitized diagnostics, and screenshots in the unique task-temporary directory from [commands.md](commands.md). Pass explicit output paths to helpers. Retain only requested deliverables or the selected final visual artifact separately before removing scratch files; never retain a credential-bearing handoff.
...
- Target ES2017-compatible JavaScript unless a newer runtime is explicitly guaranteed. Prefer classic functions where callback `this` binding or compatibility with existing script style matters, and clean up listeners, timers, intervals, and network activity in `close()`.
