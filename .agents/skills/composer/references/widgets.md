...
Use this index to choose a reference for building or editing widgets in paired Composer. Widget-specific authoring guides live in `widgets/`; this index stays at `references/widgets.md`. Read only the document matching the task. Load [graphics.md](graphics.md) when creating declarative graphics or changing layout, or when a needed value contract is not supplied by the live schema and matching guide. An isolated property edit follows [commands.md](commands.md) without loading unrelated graphics guidance. Apply the matching guide's verification requirements to the affected field and behavior, not every feature of the widget.
...
The index routes to knowledge; it does not duplicate schemas. Every widget schema is versioned and may evolve: fields can be added, removed, renamed, or redefined, and their types, defaults, runtime shapes, selections, numeric ranges, steps, formats, units, linkability, and dynamic visibility can change. Run `primitives --primitive <name>` immediately before creating a widget. Before editing an existing tile, run `get --type tile --id <id> --compact` and use the schema returned for that exact loaded version. Do not carry a field contract from documentation, memory, another tile version, or an earlier session. Re-read dynamic schemas after changing a field that can expose or hide dependent fields. Authoring references explain semantics and workflows but never override current live schema metadata. For widget-owned templates, also read [widget-subcompositions.md](widget-subcompositions.md) and [Widget Nodes](widget-nodes.md) when the owner supplies template outputs.
