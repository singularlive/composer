...
```bash
node scripts/composer-agent.js create-control --name "Layout Left" --node-type number --target layout --element-type tile --element-id <composition-tile-id> --property left --source-composition root --reuse-existing
```

The source must already exist with that exact public ID and type. Missing sources fail without creating a substitute. A conflicting target link still requires explicit `--replace` approval; reuse does not bypass ownership. A root-level sibling does not follow another module's Layout controls automatically, and following an offset below that module needs a separately verified geometry design.
