...
| `create-control --name <name> --node-type clock --target standalone` | **Targeted only:** create one native Clock stopped at zero without `--value-file`. For text output use `--tile-id <id> --property <text-field-id>` instead of standalone. Configure with `update-control`; send commands through `set-control-value`. Read [Clock](control-node-clock.md) first. |
