...
| `create-control --name <name> --node-type selection --tile-id <id> --property <field-id> --options-file <options.json> --format <text\|color\|image>` | **Targeted only:** create a named-value Selection linked to a compatible Text/Text Area, Color/Gradient, or Image field according to its format. |
