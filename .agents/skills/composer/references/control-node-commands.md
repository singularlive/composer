...
| `timer-action --id <control-id> --action <start\|play\|pause\|reset>` | Apply one legacy Time Control action with elapsed-time accounting and authoritative readback, not a Timer Control Node command. Use `set-control-value` for Timer Control Nodes. Prefer this shell-safe spelling for Time Control; `control-time` remains a compatibility alias. |
...
| `create-control --name <name> --node-type timer --target standalone` | **Targeted only:** create one native Timer stopped at zero without `--value-file`. For text output use `--tile-id <id> --property <text-field-id>` instead of standalone. Configure with `update-control`; send commands through `set-control-value`. Read [Timer](control-node-timer.md) first. |
...
| `create-control --name <name> --node-type <number\|checkbox> --target layout --element-type <tile\|group> --element-id <id> --property <layout-property> [--source-composition <root\|ancestor-id>] [--reuse-existing]` | **Targeted only:** create or reuse and link one explicitly requested Transform/Effect public control; never use as a graphic-authoring default. Exact same-public-ID/type reuse is supported for ordinary composition tiles and groups, preserves source values/metadata, and follows the same ancestor rule. It does not compute offsets or override link conflicts. |
...
`control-nodes` includes each group's `danglingChildIds`: stored membership entries that match no current public field ID, including obsolete keyIds. Inspect rather than silently discarding them. To repair an ordinary container, explicitly configure its complete intended `controlIds` using current field IDs/keyIds; verify the remaining controls and other containers afterward. This is not a Table repair API.

After `CONTROL_CONTAINER_VERIFICATION_FAILED`, inspect readback and its bounded property/expected/actual diagnostic; text metadata and URLs are redacted. Empty and missing membership are equivalent in the native store. Try a non-destructive no-op configuration first, or an authorized disposable creation probe. Never delete a container intended for recreation until creation is shown to work. Preserve the original container if creation still fails; reinstalling a compatible CLI cannot repair editor-side model verification. No raw model writes or relay execution are permitted.

Each Metric Font create/link invocation targets one explicitly named `metricfont` widget field; one source can drive several targets through successive `--reuse-existing` links. Initial creation must omit `--reuse-existing`; that flag requires an already existing exact source and is not create-or-reuse. Follow the [two-step font example and prerequisite gate](control-node-creation.md#create-versus-reuse). Linked creation copies that field's current complete value and does not accept font-selection flags. `create-controls` does not accept Metric Font entries because catalog resolution must finish before mutation. The native bulk **Connect to Metric Widgets** UI action is intentionally not exposed.
