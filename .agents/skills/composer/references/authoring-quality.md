...
- For new operator-controlled game clocks, default to the native Timer Control Node (`timer`) and link its formatted output directly to text; read [Timer Control Nodes](control-node-timer.md) before construction. Configure the sport's duration, direction and endpoint behavior explicitly. Preserve legacy Time Control (`timecontrol`) and Timer widget implementations in existing graphics unless migration is explicitly requested and permitted by the composition contract. Do not default to a free-form Text control or synthesize elapsed state from `Date.now()`.
