...
Before creating it, inspect that owner's rendered tile bounds, occupied regions, anchors, group clipping and layout node references. Ordinary composition tiles clip descendants. Reserve free space for the complete In/Out envelope inside the owner; placing a child below a fully occupied frame at `top: 100` does not extend that frame. If no space exists, agree on a root-level sibling before building it and disclose that fixed sibling geometry does not automatically follow the original parent's Layout controls. Exact layout-control reuse is available only on explicit request; see [Control Node creation](../control-node-creation.md). Do not silently resize or re-parent protected existing graphics. State In or updated DOM text is not visibility proof.
...
Create the script handoff while the actual module composition is active, or pass its ID to `script-handoff`. Let the verifier inherit that suggested target; if an explicit `--composition-id` override produces a target-mismatch warning, stop and correct the target before interpreting screenshots. A root capture is not evidence for a nested module merely because the root contains its owner.

For a text-derived module, copy [the optional text module scenario](optional-animated-module-scenario.json) to the task's temporary directory and use it with `verifyComposition.mjs --scenario-file`. Its canonical field is `Callout`; replace that payload key and the sample strings only when the inspected public contract differs. Adjust waits to the authored Timeline and Update durations before running it. The named checkpoints cover original content, UpdateOut, UpdateIn, settled replacement, clear, and parent Out on the inherited handoff target, and the scenario requires zero Player error events. View every retained image and require complete script-event counters with zero script errors before acceptance.